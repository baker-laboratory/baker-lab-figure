...
Under the **IPD/Baker profile**, all quantitative plotting, previews, exports,
and visual-QA renders use Python with Matplotlib. Do not ask for a backend,
consult a saved backend preference, or execute R/Seaborn. Retained R/Seaborn
files are upstream compatibility material only. PyMOL/ChimeraX structure
renders and explicitly requested OpenRouter AI concept schematics remain
separate allowed inputs; they do not replace the Matplotlib quantitative path.
...
| Backend exclusivity | Python / Matplotlib produced all quantitative plotting, previews, exports, and visual QA renders; no R/Seaborn code ran |
| Final size | Single-column about 89 mm or double-column about 183 mm, height not above target journal limit |
| Text size | IPD/Baker internal text starts at approximately Arial 6 pt and remains readable at final size; manuscript captions are approximately Arial 9 pt with a bold title |
| Rendered glyph floor | Every PDF text run, including math superscripts/subscripts, is at least 5 pt |
| Panel labels | Lowercase, bold, near top-left, approximately 6 pt under the IPD rule unless the journal requires another size |
...
| Color | IPD purple `#4b2e83`, cyan `#4196b5`, and magenta `#b41f85` are used where semantically appropriate; no rainbow maps, red/green-only encoding, or grayscale failure |
...
```bash
# Python source
python skills/nature-figure/scripts/validate_figure.py path/to/figure.py --profile ipd-baker

# Machine-readable report or stricter warning gate
python skills/nature-figure/scripts/validate_figure.py path/to/figure.py --profile ipd-baker --json
python skills/nature-figure/scripts/validate_figure.py path/to/figure.py --profile ipd-baker --strict
...
# Audit of the render-time Matplotlib panel-layout manifest
...
# Mandatory rendered collision audit for Matplotlib figures
...
The source preflight checks Python syntax, Matplotlib font configuration and size floor, mathtext shrinkage risk, literal legend-label capitalization, unsafe color maps, editable-text settings, vector/raster exports, DPI, common journal widths, potential sampling or unreported missing-data exclusion, simulated-data leakage, log guards, interpolation monotonicity, stochastic uncertainty encoding, rotated-text anchoring, risky annotation workarounds, and prohibited cross-backend plotting references. The panel-alignment gate reads final Matplotlib axes rectangles and applies one physical-point contract. The PDF text audit scans supported content streams for actual `Tf` font-size operators and catches reduced script glyphs that source-level `fontsize` checks miss. The rendered collision audit uses PyMuPDF geometry from the final Matplotlib PDF.

Treat the result as a deterministic source audit, not as evidence that the analysis or rendered figure is correct. Resolve all `FAIL` findings before delivery. Review every `WARN`, then run the Matplotlib source and inspect the actual SVG/PDF/TIFF/PNG outputs at final size. A warning may be acceptable only when the QA notes state the reason.
...
The bundled `panel_alignment.R` and its patchwork/gtable pathway are retained
from upstream for provenance and future merges. They are inactive under the
IPD/Baker profile and must not be invoked for production figures.
...
| `FIX BEFORE DELIVERY`, exit `1` | Correct the Matplotlib layout and rerun; do not export the delivery bundle |
...
content. It must not replace the production Matplotlib figure. Do not weaken the
...
Run only the Python / Matplotlib export block. If Matplotlib is unavailable,
stop and report the missing runtime/package instead of producing a substitute
export with R, Seaborn, or another plotting backend.
...
The declared physical canvas is part of the figure contract. Do not use
`bbox_inches="tight"` in the IPD/Baker production path because it can silently
change the exported width and height. Resolve clipping by adjusting the layout,
margins, legend placement, or text on the declared canvas, then rerun alignment,
collision, and print-size checks.
