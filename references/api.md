...
Use `DEFAULT_COLORS_IPD` as the starting point for Baker Lab/IPD figures, but
assign colors by scientific meaning and add redundant non-color cues when
needed. A target journal's mandatory or accessibility requirements may override
the suggested IPD colors.
...
```python
def apply_publication_style(font_size=6, axes_linewidth=0.8, use_tex=False):
...
**Presets:**
- IPD/Baker final-size default: `apply_publication_style(font_size=6, axes_linewidth=0.8)`
...
manuscript caption/legend, not to text drawn inside the plotting canvas.
