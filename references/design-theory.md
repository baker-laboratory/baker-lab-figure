...
**IPD/Baker profile routing:** quantitative plotting and plot revision use
Python with Matplotlib only. Do not ask the user to choose a backend, consult a
saved backend preference, or run R/Seaborn. PyMOL and ChimeraX remain valid
sources for protein-structure renders, and the explicit OpenRouter route
remains available for AI concept schematics; neither route replaces Matplotlib
for quantitative panels.

---

## 1) Typography

**IPD/Baker profile override:** at final publication size, use approximately
Arial 6 pt for internal figure text when the journal permits. Keep the 5 pt
rendered-glyph rule as a blocking floor, not the preferred default. The IPD
Arial 9 pt recommendation applies to the manuscript caption with a bold figure
title, not to the plotting canvas.
...
|---------|-----------|---------------|
| IPD/Baker journal-final figure at publication width | 6 (adjust only for journal/readability needs) | 0.8–1.2 |
...
When targeting the final dimensions of a two-column `Nature` figure page, start
at the actual publication canvas rather than a slide-sized preview. The
upstream survey's `7–9 pt` range remains context, but the IPD/Baker default is
approximately 6 pt inside the figure when readable and journal-compliant. The
9 pt recommendation applies to the manuscript caption, not to axes or legends.
...
**IPD/Baker profile override:** start from purple `#4b2e83`, cyan `#4196b5`,
and magenta `#b41f85` when those colors fit the scientific roles. Keep their
meaning stable across the paper, label them, and add marker/line/direct-label
redundancy when color alone is insufficient. Mandatory journal and
accessibility requirements take priority.
...
fig.tight_layout(pad=2)   # fix clipping through layout; keep the declared canvas
...
The declared width and height are part of the figure contract. Do not use
`bbox_inches="tight"` in the IPD/Baker production path: it recomputes the
exported bounding box and can silently change the final physical canvas. Fix
clipping by adjusting subplot geometry, margins, legend placement, or text at
the declared size, then export and inspect that exact canvas.
...
- [ ] IPD/Baker internal figure text starts at approximately Arial 6 pt; the manuscript caption is approximately Arial 9 pt with a bold title; every exported PDF glyph including scripts is ≥ 5 pt
- [ ] IPD purple `#4b2e83`, cyan `#4196b5`, and magenta `#b41f85` are used where scientifically appropriate, with stable semantics and accessible redundant encodings
...
- [ ] Final physical width and height are declared before plotting and remain unchanged at export; clipping is fixed through layout rather than `bbox_inches="tight"`
