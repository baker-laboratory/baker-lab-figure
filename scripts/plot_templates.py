...
def save_bundle(fig: Any, output: Path) -> dict[str, str]:
    ...
    paths = {
        ...
        "alignment_json": str(output.with_suffix(".alignment.json")),
        "alignment_svg": str(output.with_suffix(".alignment.svg")),
    }
    excluded_axes = [axis for axis in fig.axes if axis.get_label() == "<colorbar>"]
    require_matplotlib_panel_alignment(
        fig,
        json_out=paths["alignment_json"],
        overlay_svg=paths["alignment_svg"],
        exclude_axes=excluded_axes,
        tolerance_pt=1.5,
        gutter_tolerance_pt=1.5,
        strict=True,
    )
    # Preserve the physical canvas selected before plotting. Using
    # bbox_inches="tight" here would silently change the delivered dimensions.
...
def plot_volcano(args: argparse.Namespace) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    ax.scatter(effects[down], scores[down], s=7, color=IPD_PALETTE["cyan"], alpha=0.72, edgecolors="none", rasterized=rasterized, label=f"Down ({down.sum()})")
    ax.scatter(effects[up], scores[up], s=7, color=IPD_PALETTE["magenta"], alpha=0.72, edgecolors="none", rasterized=rasterized, label=f"Up ({up.sum()})")
...
def plot_dotplot(args: argparse.Namespace) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    cmap = LinearSegmentedColormap.from_list(
        "ipd_expression",
        [IPD_PALETTE["cyan"], "#F7F7F7", IPD_PALETTE["magenta"]],
    )
...
def plot_paired(args: argparse.Namespace) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    for patch, color in zip(
        boxes["boxes"],
        (IPD_PALETTE["cyan"], IPD_PALETTE["magenta"]),
    ):
        patch.set(facecolor=color, edgecolor="#555555", alpha=0.55, linewidth=0.7)
    ax.scatter(np.zeros_like(values_a), values_a, s=13, color=IPD_PALETTE["cyan"], edgecolors="white", linewidths=0.35, zorder=3)
    ax.scatter(np.ones_like(values_b), values_b, s=13, color=IPD_PALETTE["magenta"], edgecolors="white", linewidths=0.35, zorder=3)
